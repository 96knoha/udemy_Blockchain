...
def find_neighbours(my_host, my_port, star_ip_range, end_ip_range, start_port, end_port):
    #192.168.0.24 (1,3)
    ...
    for guess_port in range(start_port, end_port):
        for ip_range in range(star_ip_range, end_ip_range):
...
def get_host():
    try:
        ...
        logger.debug({'action': 'get_host', 'error': ex})
        return '127.0.0.1'
